...
import com.harish.repository.PlanRepository;
import com.harish.repository.UserRepository;
import com.harish.security.AuthUtil;
import com.harish.service.PaymentProcessor;
...
public class StripePaymentProcessor implements PaymentProcessor {
    ...
    public void handleWebhookEvent(String type, StripeObject stripeObject, Map<String, String> metadata) {
        ...
        switch (type) {
            ...
            default -> log.info("Ignoring the event: {}", type);
        }
    ...
    private void handleCheckoutSessionCompleted(Session session, Map<String, String> metadata) {
        ...
        log.info("Processing checkout.session.completed with metadata: {}", metadata);
        if (metadata == null || !metadata.containsKey("user_id") || !metadata.containsKey("plan_id")) {
            log.warn("Missing user_id or plan_id in session metadata. Metadata received: {}", metadata != null ? metadata.keySet() : "null");
            return;
        }
        try {
            ...
            log.info("Extracted from metadata: userId={}, planId={}, subId={}, customerId={}", userId, planId, subscriptionId, customerId);
            User user = getUser(userId);
            if (user.getStripeCustomerId() == null) {
                user.setStripeCustomerId(customerId);
                userRepository.save(user);
                log.info("Updated user {} with stripe customer id {}", userId, customerId);
            }
            subscriptionService.activateSubscription(userId, planId, subscriptionId, customerId);
            log.info("Successfully finished activation call for subscription {} for user {}", subscriptionId, userId);
        } catch (Exception e) {
    ...
    private void handleCustomerSubscriptionUpdated(Subscription subscription) {
        ...
        try {
            if (subscription.getItems() == null || subscription.getItems().getData() == null || subscription.getItems().getData().isEmpty()) {
                log.warn("No items found in subscription {}", subscription.getId());
                return;
            }
            ...
            log.info("Updated subscription {} with status {}", subscription.getId(), status);
        } catch (ResourceNotFoundException e) {
            log.warn("Subscription not found for update, ignoring: {}", e.getMessage());
        } catch (Exception e) {
            log.error("Error processing subscription updated event", e);
        }
    ...
    private void handleCustomerSubscriptionDeleted(Subscription subscription) {
        ...
        try {
            ...
            log.warn("Subscription not found for deletion, ignoring: {}", e.getMessage());
        }
    ...
    private void handleInvoicePaid(Invoice invoice) {
        ...
        try {
            ...
            if (subscription.getItems() != null && subscription.getItems().getData() != null && !subscription.getItems().getData().isEmpty()) {
                ...
                log.info("Renewed subscription {}", subId);
            }
        } catch (ResourceNotFoundException e) {
            log.warn("Subscription not found for renewal, ignoring: {}", e.getMessage());
        } catch (StripeException e) {
            log.error("Stripe API error on invoice.paid", e);
        }
    ...
    private void handleInvoicePaymentFailed(Invoice invoice) {
        ...
        try {
            ...
            log.info("Marked subscription {} as past due", subId);
        } catch (ResourceNotFoundException e) {
            log.warn("Subscription not found to mark past due, ignoring: {}", e.getMessage());
        }
